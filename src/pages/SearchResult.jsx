...
export default function SearchResult() {
  ...
  return (
    <div className="max-w-[1400px] mt-[20px] lg:mt-[65px] mx-auto lg:px-4 px-2 py-10 min-h-screen">
      {/* --- HEADER SECTION --- */}
      <div className="flex flex-col md:flex-row md:items-center justify-between md:gap-4 gap-2 md:mb-5 mb-2.5 bg-white border border-slate-200 rounded p-3 shadow-xs">
        <div className="flex items-center gap-3">
          <div className="w-1.5 h-6 bg-[#fe741d] rounded" />
          <h1 className="text-xs md:text-sm font-medium text-slate-800 uppercase tracking-wider">
            Registry Query:{" "}
            <span className="text-[#fe741d]">"{decodedKeyword}"</span>
          </h1>
        </div>

        <div className="flex items-center gap-2 px-4 py-1.5 bg-slate-50 border border-slate-100 rounded">
          <div
      ...
      {/* --- RESULTS GRID --- */}
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 bg-slate-50 rounded border border-dashed border-slate-200">
          ...
          <p className="text-slate-400 text-sm font-medium uppercase tracking-[0.3em] text-center px-4">
            Search string returned zero results.
          </p>
          <button
            onClick={() => navigate("/")}
            className="mt-6 px-8 py-3 bg-white border border-slate-200 rounded-xl text-[11px] font-medium text-indigo-600 uppercase tracking-widest hover:bg-indigo-50 transition-all shadow-sm"
          >
            Reset Terminal
          </button>
        </div>
